function reloadGraphData() {
  ...
  // Process uevents to update each variable
  ...
  // If user has stats active
  if ( userdata.stats == 1 ) {

   // Show stats table
   $("#statistics_container").removeClass("hidden");

   // If user has input effect active
   ...
   if ( $("#datepicker-01").val() != "" && $("#timepicker-01").val() != "" ) {
      
     predata.addColumn('number', 'Real BG mg/dl');
     
     for (i=0;i<simt/5;i++) {
        if ( userdata.inputeffect == 1 ){
          predata.addRow([(dt*i)+1,simbg[i],userdata.bginitial+simbgc[i],userdata.bginitial+simbgi[i],stats[0], stats[2], stats[3], livedata[i]]);
        }else{
          predata.addRow([(dt*i)+1,simbg[i],stats[0], stats[2], stats[3], livedata[i]]);
        }
     }
   ...
   if ( $("#datepicker-01").val() != "" && $("#timepicker-01").val() != "" ) {
     ...
     for (i=0;i<simt/5;i++) {
       ...
       if ( userdata.inputeffect == 1 ) {
         predata.addRow([(dt*i)+1,simbg[i],userdata.bginitial+simbgc[i],userdata.bginitial+simbgi[i],livedata[i]]);
       }else{
         predata.addRow([(dt*i)+1,simbg[i],livedata[i]]);
       }
     }
...
function showUserData() {
  ...
  $("#step_two_userdata").toggleClass("hidden");
  ...
  if ( $("#settings").hasClass("disabled") ) {} else {
    $("#settings").toggleClass("disabled")
  }

  if ( $("#step_two_settings").hasClass("hidden") ) {} else {
    $("#step_two_settings").toggleClass("hidden")
  }
  
  if ( $("#step_two_userdata").hasClass("hidden") ) {
    $("#add_insulin").toggleClass("disabled");        
    $("#add_carbs").toggleClass("disabled");                        
    $("#settings").toggleClass("disabled");                        
  }
...
function loadRealData() {
  ...
  if ( $("#datepicker-01").val() != "" && $("#timepicker-01").val() != "" ) {
    ...
    $("#realdata_loading").removeClass("hidden");
    
    var userdata = JSON.parse(localStorage["userdata"]);
    
    var year = $("#datepicker-01").datepicker('getDate').getFullYear();
    var month = $("#datepicker-01").datepicker('getDate').getMonth();
    var day = $("#datepicker-01").datepicker('getDate').getDate();
    var hours = new Date($("#timepicker-01").timepicker('getTime')).getHours();
    var minutes = new Date($("#timepicker-01").timepicker('getTime')).getMinutes();
  
    var time_start = new Date(year,month,day,hours,minutes).getTime();
    var time_end = new Date(year,month,day,hours,minutes).setMinutes(new Date(year,month,day,hours,minutes).getMinutes() + (userdata.simlength*60)+30 );

  	var key=$("#api_key").val();
  	var url=$("#api_url").val()+"?&apiKey="+key+"&q={'date':{'$gte':" + time_start + ",'$lte':"+ time_end +"}}";
  	...
  	$.ajax({
        ...
        // Update n
        var n=Math.floor((userdata.simlength*60)/5)-1; //points in simulation
        
        if ( data.length > 0 ) {
          
          // Set initial bg data to real data
          userdata.bginitial = data[0].sgv;      
          ...
          // Populate real data array
          for (i=0;i<n;i++) {
            if ( data[i] && data[i].sgv > 30 ) {
              livedata[i] = data[i].sgv;              
            } else {
              livedata[i] = null
            }
          }
          
          $("#realdata_loading").addClass("hidden");
          $("#realdata_loading h5").html("Loading");        
          
        } else {
          
          $("#realdata_loading h5").html("No real data to display!<br/>Please change initial time");
...
function showUserSettings() {
  ...
  if ( $("#userdata").hasClass("disabled") ) {} else {
    $("#userdata").toggleClass("disabled")
  }

  if ( $("#step_two_userdata").hasClass("hidden") ) {} else {
    $("#step_two_userdata").toggleClass("hidden")
  }
...
// Update Live Data if it is present!

var updateRealDataCheck = setInterval(function(){ updateRealData() }, 300000);
