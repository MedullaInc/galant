...
class SignedOutTest(StaticLiveServerTestCase):
    ...
    def test_can_signup_int(self):
        ...
        for lang, e_text in [('en', u'Request FREE Early Access'),
                             # ('es', u'Regístrate!')
                             ]:
            activate(lang)
    ...
    def test_page_blocked(self):  # pragma: no cover
        for view_name in get_resolver(None).reverse_dict.keys():
            ...
            if view_name in ['edit_client', 'client_detail', 'add_service', 'edit_quote',
                             ...
                             'api_project_detail', 'api_client_detail', 'api_note_detail',
                             'api_quote_template_detail', 'api_brief_template_detail',
                             'api_question_detail', 'client_work_detail', 'client_money_detail', 'quote_text']:
