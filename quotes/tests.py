...
class QuoteFormTest(test.TestCase):
    ...
    def test_new_service(self):
        new_data = {'-service-2-name': 'title123', '-service-2-type': '3',
                    '-service-2-description': 'title123', '-service-2-cost': '3',
                    '-service-2-quantity': '2', '-service-2-service_name': 'title123',}
        new_data.update(self.data)
    ...
    def test_same_sections(self):
        ...
        f.cleaned_data['-section-0-id'] = intro_id
        f.cleaned_data['-section-1-id'] = margin_id
        f.cleaned_data['-section-2-id'] = section_ids[2]
        new_obj = qf.create_quote(f)
        new_obj.save()
    ...
    def test_modify_section(self):
        ...
        f.cleaned_data['-section-2-title'] = 'new title'
        new_obj = qf.create_quote(f)
        new_obj.save()
        new_section_ids = [s.id for s in new_obj.sections.all()]
